...
# Function to mirror landmarks for left-right hand variation
def mirror_landmarks(landmarks):
    mirrored_landmarks = landmarks.copy()
    for i in range(0, len(landmarks), 3):
        mirrored_landmarks[i] = 1 - landmarks[i]  # Invert the x-coordinate
    ...
    while captured_samples < samples_per_gesture:
        ...
        if results.multi_hand_landmarks:
            ...
            if landmarks.size > 0:
                ...
                # Add mirrored landmarks to handle other hand variation
                mirrored_landmarks = mirror_landmarks(normalized_landmarks)
                data.append(mirrored_landmarks)
                labels.append(gesture_names[gesture_index])
